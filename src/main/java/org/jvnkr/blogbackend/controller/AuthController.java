...
public class AuthController {
  ...
  @PostMapping("/login")
  public ResponseEntity<JwtAuthResponseDto> login(@RequestBody LoginDto userDto) {
    JwtAuthResponseDto tokens = authService.login(userDto);
  ...
  public ResponseEntity<SessionTokenDto> validate(@CookieValue(value = "a_t", defaultValue = "") String accessToken,
    ...
    return newToken == null ? ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null)
            : ResponseEntity.status(HttpStatus.OK).body(newToken);
  }
}
